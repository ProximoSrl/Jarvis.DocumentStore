Write-Output "Installing Jarvis.DocumentStore service. Script running from $PSScriptRoot"
...
# Default the values
$installationRoot = "c:\jarvis\setup\DocumentStore\ConfigurationManagerHost"
...
$artifactFile = "$PSScriptRoot\..\Artifacts\Jarvis.DocumentStore.Host.zip"
$artifactJobsFile = "$PSScriptRoot\..\Artifacts\Jarvis.DocumentStore.Jobs.zip"
...
if(!(Test-Path -Path "$PSScriptRoot\DocumentStoreSetup.ps1" ))
{
     Throw "Unable to find package file $PSScriptRoot\DocumentStoreSetup.ps1"
}
...
& $PSScriptRoot\DocumentStoreSetup.ps1 -deployHostFile $artifactFile -deployJobsFile $artifactJobsFile -installationRoot $installationRoot