...
namespace Jarvis.DocumentStore.JobsHost.Support
{
    /// <summary>
    /// 
    /// DUPLICATED FROM DOCUMENTSTORE HOSTS, TO MINIMIZE DEPENDENCY OF SHARED FROM LOG4NET
    /// 
    /// 
    ...
    internal class UdpAppender : AppenderSkeleton
    {
        ...
        /// <summary>
        /// GianMaria: Fix for UdpAppender, with Framework4 it is not possible to use
        /// 127.0.0.2 for RemoteAddress, so I decided to pass parameter as string value 
        /// then simply catch the format exception and fix trying to use the standard
        /// IPV4 addresses.
        /// </summary>
        /// <returns></returns>
        public IPAddress GetRemoteAddress()
        {
            try
            {
                return IPAddress.Parse(RemoteAddress);
            }
        ...
        public override void ActivateOptions()
        {
            ...
            {
                this.RemoteEndPoint = new IPEndPoint(this.GetRemoteAddress(), this.RemotePort);
...
#else
                    this.Client = new UdpClient(GetRemoteAddress().AddressFamily);
#endif
                }
                else
                {
...
#else
                    this.Client = new UdpClient(this.LocalPort, GetRemoteAddress().AddressFamily);
#endif
                }
            }
            catch (Exception ex)
