...
namespace Jarvis.DocumentStore.Jobs.MsGraphOfficeToPdf
{
    ...
    /// </summary>
    public class MsGraphOfficeToPdfPdfOutOfProcessJob : AbstractOutOfProcessPollerJob
    {
        private readonly MsGraphOfficeToPdfConverter _msGraphOfficeToPdfConverter;

        public MsGraphOfficeToPdfPdfOutOfProcessJob(
            MsGraphOfficeToPdfConverter msGraphOfficeToPdfConverter)
        {
            PipelineId = "office";
            QueueName = "office";
        ...
        protected async override Task<ProcessResult> OnPolling(PollerJobParameters parameters, string workingFolder)
        {
            ...
            var conversionResult = await _msGraphOfficeToPdfConverter.ConvertFileAsync(pathToFile, workingFolder);
